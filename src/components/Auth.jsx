...
export default function Auth({ onAuthSuccess, redirectPath }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        const notEnabled = /provider|not enabled|unsupported/i.test(error.message || "");
        setErrorMsg(notEnabled
          ? "Login con Google non ancora attivo su questo sito. Per ora usa email e password."
          : error.message);
        setLoading(false);
        return;
      }
